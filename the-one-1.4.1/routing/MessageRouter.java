...
public abstract class MessageRouter {
	...
	public void init(DTNHost host, List<MessageListener> mListeners) {
		this.incomingMessages = new HashMap<String, Message>();     //nantinya akan digunakan untuk menyimpan pesan yang akan datang
		this.messages = new HashMap<String, Message>();             //nantinya akan digunakan untuk menyimpan pesan yang dibawa//carrying
		this.deliveredMessages = new HashMap<String, Message>();    //nantinya akan digunakan untuk menyimpan pesan yang datang apakah dia sebagai penerima merupakan final destinasinya
	...
	public int receiveMessage(Message m, DTNHost from) {
		...
		this.putToIncomingBuffer(newMessage, from);		//sedang datang di buffer//
		newMessage.addNodeOnPath(this.host);
		
		for (MessageListener ml : this.mListeners) {
			ml.messageTransferStarted(newMessage, from, getHost()); //sudah terjadi, sudah berlangsung di transfer//
		}
	...
	public Message messageTransferred(String id, DTNHost from) {
		...
		if (!isFinalRecipient && outgoing!=null) {    //cek apakah dia merupakan final destinasi/tujuan pesan
                    // not the final recipient and app doesn't want to drop the message
                    // -> put to buffer ->                    //buffer disebut Message
                    addToMessages(aMessage, false);
		}
		else if (isFirstDelivery) {                   //cek apakah dia merupakan tujuan dari pesan?
                    this.deliveredMessages.put(id, aMessage); //kalau true dia berarti sudah menerima pesan sebelumnya//
		}                                             //kalau flase dia berarti belum menerima pesan sebelumnya//
