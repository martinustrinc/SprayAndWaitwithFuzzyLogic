 ...
 */
public class SprayAndWaitRouterWithFuzzyGlobalKnowledgeHaggle3Infocom5 implements RoutingDecisionEngineSnW {
    ...
    public SprayAndWaitRouterWithFuzzyGlobalKnowledgeHaggle3Infocom5(Settings s) {
    ...
    public SprayAndWaitRouterWithFuzzyGlobalKnowledgeHaggle3Infocom5(SprayAndWaitRouterWithFuzzyGlobalKnowledgeHaggle3Infocom5 prototype) {
    ...
    public boolean isFinalDest(Message m, DTNHost thisHost) {
        /** if in the front is the destination node */
...
//        return m.getTo() != thisHost;
    ...
    public boolean shouldSendMessageToHost(Message m, DTNHost otherHost) { //sender
        ...
        SprayAndWaitRouterWithFuzzyGlobalKnowledgeHaggle3Infocom5 peer = getOtherFuzzyRouter(otherHost);
    ...
    public RoutingDecisionEngineSnW replicate() {
        return new SprayAndWaitRouterWithFuzzyGlobalKnowledgeHaggle3Infocom5(this);
    }
    

    private SprayAndWaitRouterWithFuzzyGlobalKnowledgeHaggle3Infocom5 getOtherFuzzyRouter(DTNHost host) {
        MessageRouter otherRouter = host.getRouter();
        assert otherRouter instanceof DecisionEngineRouterSnW : "This router only works "
                + " with other routers of same type";

        return (SprayAndWaitRouterWithFuzzyGlobalKnowledgeHaggle3Infocom5) ((DecisionEngineRouterSnW) otherRouter).getDecisionEngine();
    }
    ...
    private double contactTimeDuration_VeryLowFunciton(DTNHost host) {
        if (getCTDFor(host) <= 60) {
            return 1.0;
        } else if (60 < getCTDFor(host) && getCTDFor(host) < 70) {
            return (70 - getCTDFor(host)) / (70 - 60); 
        } else {
            return 0;
        }
    }
    private double contactTimeDuration_LowFunciton(DTNHost host) {
        if (60 < getCTDFor(host) && getCTDFor(host) < 70) {
            return (getCTDFor(host) - 60)/(70 - 60);
        } else if (120 < getCTDFor(host) && getCTDFor(host) < 130) {
            return (130 - getCTDFor(host))/(130 - 120);
        } else if (70 <= getCTDFor(host) && getCTDFor(host) <= 120) {
            return 1.0;
        } else {
            return 0;
        }
    ...
    private double contactTimeDuration_MediumFunciton(DTNHost host) {
        if (120 < getCTDFor(host) && getCTDFor(host) < 130) {
            return (getCTDFor(host) - 120)/(130 - 120);
        } else if (180 < getCTDFor(host) && getCTDFor(host) < 190) {
            return (190 - getCTDFor(host))/(190 - 180);
        } else if (130 <= getCTDFor(host) && getCTDFor(host) <= 180) {
            return 1.0;
        } else {
            return 0;
        }
    ...
    private double contactTimeDuration_HighFunciton(DTNHost host) {
        if (180 < getCTDFor(host) && getCTDFor(host) < 190)
            return (getCTDFor(host) - 180)/(190 - 180);
        else if (240 < getCTDFor(host) && getCTDFor(host) < 250 )
            return (250 - getCTDFor(host))/(250 - 240);
        else if (190 <= getCTDFor(host) && getCTDFor(host) <= 240) 
            return 1.0;
        else return 0;
    }
    private double contactTimeDuration_VeryHighFunciton(DTNHost host) {
        if (240 < getCTDFor(host) && getCTDFor(host) < 250) {
            return (getCTDFor(host) - 240)/(250 - 240);
        } else if (getCTDFor(host) >= 250) {
            return 1;
        } else {
            return 0;
        }
    ...
    private double IntercontactTimeDuration_VeryLowFunciton(DTNHost host) {
        if (getITDFor(host) <= 4000) {
            return 1.0;
        } else if (4000 < getITDFor(host) && getITDFor(host) < 5000) {
            return (5000 - getITDFor(host)) / (5000 - 4000);
        } else {
            return 0;
        }
    }
    private double IntercontactTimeDuration_LowFunciton(DTNHost host) {
        if (4000 < getITDFor(host) && getITDFor(host) < 5000) {
            return (getITDFor(host) - 4000)/(5000 - 4000);
        } else if (9000 < getITDFor(host) && getITDFor(host) < 10000) {
            return (10000 - getITDFor(host))/(10000 - 9000);
        } else if (5000 <= getITDFor(host) && getITDFor(host) <= 9000) {
            return 1.0;
        } else {
            return 0;
        }
    ...
    private double IntercontactTimeDuration_MediumFunciton(DTNHost host) {
        if (9000 < getITDFor(host) && getITDFor(host) < 10000) {
            return (getITDFor(host) - 9000)/(10000 - 9000);
        } else if (14000 < getITDFor(host) && getITDFor(host) < 15000) {
            return (15000 - getITDFor(host))/(15000 - 14000);
        } else if (10000 <= getITDFor(host) && getITDFor(host) <= 14000) { 
            return 1.0;
        } else {
            return 0;
        }
    ...
    private double IntercontactTimeDuration_HighFunciton(DTNHost host) {
        if (14000 < getITDFor(host) && getITDFor(host) < 15000) {
            return (getITDFor(host) - 14000)/(15000 - 14000);
        } else if (19000 < getITDFor(host) && getITDFor(host) < 20000) {
            return (20000 - getITDFor(host))/(20000 - 19000);
        } else if (15000 <= getITDFor(host) && getITDFor(host) <= 19000) {
            return 1.0;
        } else { 
            return 0;
        }
    ...
    private double IntercontactTimeDuration_VeryHighFunciton(DTNHost host) {
        if (19000 < getITDFor(host) && getITDFor(host) < 20000) {
            return (getITDFor(host) - 19000)/(20000 - 19000);
        } else if (getITDFor(host) >= 20000) {
            return 1;
        } else {
            return 0;
        }
